...
def test_social_networks_get_image(image_url, status_code, message_type, message, access_token):
    ...
    if response.status_code == 200:
        ...
        parsed_data = response.json()
        logger.info(parsed_data)
        assert parsed_data[message_type] == message, f"Expected message '{message}', got '{parsed_data[message_type]}'"
