...
def test_find_friend_verify_user_search_friend_request(signed_in_driver):
    # ---------- Step 1: Navigate to the GreenCity URL
    main_page = MainPage(signed_in_driver)
    ...
    # Step 2: Click on the "My Space" section in the header
    ...
    # Step 3: Click in the "+" icon in the "My Friends" section within the profile information banner
    ...
    # Step 4: Enter the target username into the Search input field
    ...
    # Step 5: Click the "Add Friend" button on the found user's card
    ...
    assert find_friend_page.get_friend_request_sent_msg() == "Friend request sent", \
    ...
    # Step 7: Enter the same target username into the Search input field.
    find_friend_page.search_friend(target_user_name)
    ...
    # Step 9: CLick on the "Cancel Request" button.
    ...
    # Step 10: Navigate back to the previous page ("Find Friend" page).
    user_all_habits_page.go_back()
    find_friend_page = FindFriendPage(signed_in_driver)

    # Expected result:
    # The "Find Friend" page loads successfully.
    # The Search input field is cleared, and the full list of users is displayed.
    find_friend_page.is_page_loaded(), "'Find Friend' page did not load successfully."

    # Step 11: Enter the same target username into the Search input field.
    find_friend_page.search_friend(target_user_name)
    user_friend_card = find_friend_page.get_friend_card_by_name(target_user_name)
