...
Feature: Add Eco News to Favorites by ID
  ...
  As a user
  I want to add an eco news item to my favorites by its ID
  ...
  Scenario Outline: Add eco news to favorites by id
    When I send a POST request to "<news_id>/favorites"
  ...
  Scenario: Successfully add eco news to favorites
  ...
  Scenario: User has no permission to add eco news to favorites
    Given a valid eco news id
    When I send a POST request to "<news_id>/favorites"
  ...
  Scenario: Unexpected status code
    When I send a POST request to "<news_id>/favorite"
