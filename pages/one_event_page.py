...
class OneEventPage(BasePage):
    ...
    def event_info_block(self):
        ...
        event_info_element = self.driver.find_element(*self.event_info_block_locator)
        from components.event_info_component import EventInfoComponent  # pylint: disable=import-outside-toplevel
        return EventInfoComponent(event_info_element)

    def social_media_links(self):
        """Get the social media links component."""
        social_media_links = self.driver.find_element(*self.event_social_media_links_locator)
        from components.social_media_links_component import SocialMediaLinksComponent  # pylint: disable=import-outside-toplevel
        return SocialMediaLinksComponent(social_media_links)

    def save_event(self):
        """Click the button to save the event if it's not already saved."""
        save_button_element = self.driver.find_element(*self.save_event_button_locator)
    ...
    def join_event(self):
        ...
        join_button_element = self.driver.find_element(*self.join_event_button_locator)
