...
class FriendAbstractPage(BasePage):
    ...
    def click_all_friends_tab(self) -> "AllFriendsPage":
        ...
        from pages.abstract_pages.friend_abstract_users.all_friends_page import (
            AllFriendsPage,  # pylint: disable=import-outside-toplevel
        )
    ...
    def has_user_banner(self) -> bool:
        ...
        try:
            self.get_wait().until(EC.visibility_of_element_located(
                self.locators["user_info_banner"][:2])
            )
            return True
